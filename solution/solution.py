...
def call_openai(
    ...
    return (responses.choices[0].message.content, latency_seconds)    
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        messages = history + [{"role": "system", "content": "Bạn là trợ giảng thân thiện."}]
...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        tokens_info = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
        total_tokens += tokens_info["input_tokens"] + tokens_info["output_tokens"]
        total_cost += tokens_info["input_cost"] + tokens_info["output_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        prompt = result.get("prompt", "")[:40]
        gpt4o_response = result.get("gpt4o_response", "")[:40]
        mini_response = result.get("mini_response", "")[:40]
        gpt4o_latency = f"{result.get('gpt4o_latency', 0):.2f}"
        mini_latency = f"{result.get('mini_latency', 0):.2f}"
